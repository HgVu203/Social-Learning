...
export const usePostMutations = () => {
  ...
  const likeComment = useMutation({
    mutationFn: async ({ postId, commentId }) => {
      console.log(`Toggling like on comment ${commentId} on post ${postId}`);
      const response = await axiosService.post(
    ...
    onError: (error) => {
      ...
      // Bỏ hiển thị thông báo lỗi
      // Toast.error(error.response?.data?.error || "Failed to toggle comment like");
    },
  });
